...
static void ticket_spinlock_lock(ticket_spinlock_t *lock) {
    uint_fast16_t ticket = atomic_fetch_add_explicit(&lock->ticket, 1, memory_order_acq_rel);
...
static bool ticket_spinlock_trylock(ticket_spinlock_t *lock) {
    uint_fast16_t ticket = atomic_load_explicit(&lock->ticket, memory_order_relaxed);
    uint_fast16_t now_serving = atomic_load_explicit(&lock->now_serving, memory_order_relaxed);
    if (ticket != now_serving) return false;

    uint_fast16_t next_ticket = ticket + 1;
...
static bool ticket_spinlock_is_locked(ticket_spinlock_t *lock) {
    uint_fast16_t ticket = atomic_load(&lock->ticket);
    uint_fast16_t now_serving = atomic_load(&lock->now_serving);
